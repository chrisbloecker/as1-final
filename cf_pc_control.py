...
class ControllerThread(threading.Thread):
    ...
    def run(self):
        ...
        ix         = 0   # index for waypoint the drone should reach

        self.pos_ref   = sequence[ix][0]
        ...
        with open(log_file_name, 'w') as fh:
            ...
            while True:
                ...
                self.calc_control_signals(stop_motor)
                ...
                if np.linalg.norm(self.pos_ref - self.pos) < tolerance and not position_found:
                  position_found = True
                  stop_motor     = sequence[ix][1] == "stop"
                  hold_until     = time_start + sequence[ix][2]
                  print("Holding position for {:n} seconds".format(sequence[ix][2]))

                if time_start >= hold_until and position_found:
                  position_found = False
                  stop_motor     = False
                  if ix < len(sequence) + 1:
                    ix += 1
                    self.pos_ref = sequence[ix][0]
                    print("Setting reference to index {:n}: {:s}".format(ix, str(self.pos_ref)))
                  else:
                    self.pos_ref[2] = 0.0
    ...
    def calc_control_signals(self, stop = False):
        ...
        message = ('ref: ({}, {}, {}, {})\n'.format(self.pos_ref[0], self.pos_ref[1], self.pos_ref[2], self.yaw_ref)+
                   ...
                   'error: ({}, {}, {})\n'.format(ex, ey, ez)+
                   'integral: ({}, {}, {})\n'.format(self.ex_int, self.ey_int, self.ez_int)+
...
class DistanceCommand(Command):
    ...
    def execute(self, drone):
        drone.setRelativeTarget(dx, dy, dz)
